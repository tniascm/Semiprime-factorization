...
fn factor_nfs_inner(n: &Integer, params: &NfsParams, variant: u32, pre_poly: Option<&gnfs::types::PolynomialPair>, ext_polyselect_ms: f64) -> NfsResult {
    ...
    // With singleton-prune guarded on excess, SQs beyond lim1 are safe:
    // they create SQ columns but won't cascade-delete rows when excess is low.
    // No automatic q-cap. Let the adaptive system decide when enough rels.
    ...
    loop {
        ...
        // Check timeouts
        // Use wall clock from pipeline start for timeout (accumulates across polys)
        let sieve_elapsed_ms = pipeline_start.elapsed().as_millis() as u64;
